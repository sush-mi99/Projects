		...
		do {
			...
			case 1:
				...
				try {
					...
					if (cust_phno.matches("[0-9]{10}") && cust_emailid.matches(regex)) {
						customer = new Customer( cust_name, cust_emailid, cust_phno, cust_id, cust_pwd);
					} else {
						throw new BankException("Check your MobileNumber and EmailId");
					}
					customer = bankCrudService.registerAccount(customer);
					if (customer != null) {
						log.info("Successfully registered to Salvatore Bank");
						log.info("\n");
						log.info(customer);
						log.info("\n");
					}
